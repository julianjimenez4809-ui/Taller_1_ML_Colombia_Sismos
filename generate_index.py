    ...
    <div id="tabs" class="sticky top-0 z-50 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/10">
        <div class="max-w-7xl mx-auto px-6 overflow-x-auto no-scrollbar">
            <div class="flex space-x-8 text-sm font-medium font-body whitespace-nowrap" role="tablist">
                <button class="tab-btn active py-4 border-b-2 border-[#7b39fc] text-[#7b39fc]" data-target="overview" id="overview-tab" onclick="switchTab('overview')">1. Overview</button>
                <button class="tab-btn py-4 border-b-2 border-transparent text-gray-400 hover:text-white" data-target="eda" id="eda-tab" onclick="switchTab('eda')">2. EDA</button>
                <button class="tab-btn py-4 border-b-2 border-transparent text-gray-400 hover:text-white" data-target="modeling" id="modeling-tab" onclick="switchTab('modeling')">3. Modelado (K-Means)</button>
                <button class="tab-btn py-4 border-b-2 border-transparent text-gray-400 hover:text-white" data-target="results" id="results-tab" onclick="switchTab('results')">4. Resultados</button>
                <button class="tab-btn py-4 border-b-2 border-transparent text-gray-400 hover:text-white" data-target="deployment" id="deployment-tab" onclick="switchTab('deployment')">5. Impacto SGC</button>
            </div>
        </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-16">
        ...
        <section id="modeling" class="tab-content space-y-16">
             ...
             <div class="animate-up">
                 ...
                 <p class="text-gray-400 text-sm mb-4">Para validar nuestro campeonato, forzamos a K-Means a combatir en un ring de modelos contra el mundo de probabilidades (GMM) y las búsquedas locales basadas en densidad (DBSCAN).</p>
                 
                 <div class="space-y-8 mb-8">
                     <!-- KMeans -->
                     <div class="glass-card flex flex-col md:flex-row gap-6 p-6 items-center">
                         <div class="md:w-1/3 text-left space-y-3">
                             <h4 class="text-[#7b39fc] font-bold text-xl">1. K-Means (El Campeón 👑)</h4>
                             <p class="text-white text-sm"><b>Metodología:</b> Distancia Euclidiana Plana y partición vectorial por centroides.</p>
                             <p class="text-gray-400 text-sm">Logra la hazaña perfecta que busca el SGC: crea polígonos fronterizos secos, tajantes y sin ambigüedades. Permite evacuar ciudades con límites claros, haciéndolo el ganador indiscutible.</p>
                         </div>
                         <div class="md:w-2/3 w-full h-[450px]" id="algo-kmeans"></div>
                     </div>

                     <!-- GMM -->
                     <div class="glass-card flex flex-col md:flex-row gap-6 p-6 items-center">
                         <div class="md:w-1/3 text-left space-y-3">
                             <h4 class="text-white font-bold text-xl">2. Gaussian Mixture (GMM)</h4>
                             <p class="text-white text-sm"><b>Metodología:</b> Probabilidad de Elipses Anidadas mediante Co-varianza.</p>
                             <p class="text-gray-400 text-sm">Crea agrupaciones borrosas "tipo huevo gigante" que se entrelazan suavemente. Excelente conceptualmente, pero muy peligroso operativamente para emitir alertas públicas que requieren límites de evacuación binarios.</p>
                         </div>
                         <div class="md:w-2/3 w-full h-[450px]" id="algo-gmm"></div>
                     </div>

                     <!-- DBSCAN -->
                     <div class="glass-card flex flex-col md:flex-row gap-6 p-6 items-center">
                         <div class="md:w-1/3 text-left space-y-3">
                             <h4 class="text-[#ef4444] font-bold text-xl">3. DBSCAN (Fracaso)</h4>
                             <p class="text-white text-sm"><b>Metodología:</b> Búsqueda expansiva pura de densidad topológica.</p>
                             <p class="text-gray-400 text-sm">Falla miserablemente. Al ser los Andes una cadena continua ininterrumpida de hiper-sismos, DBSCAN se vuelve loco y traga todo Colombia fusionándolo en un solo "Súper-Continente", excluyendo al mar como ruido nulo.</p>
                         </div>
                         <div class="md:w-2/3 w-full h-[450px]" id="algo-dbscan"></div>
                     </div>
                 </div>
                 
                 <!-- Table for SEO/Accessibility retained but simplified as visual is now dominant above -->
             </div>
        </section>
        ...
        <section id="results" class="tab-content space-y-16">
            ...
            <!-- Tactical Map -->
            <div class="glass-card p-0 overflow-hidden animate-up max-w-[1200px] mx-auto text-center border-t-8 border-t-[#ef4444]">
                <div class="p-6 pb-2">
                    <h3 class="font-display text-2xl font-bold mb-4">MAPA TÁCTICO MILITAR 2D: RIESGO ACUMULADO POR PLACA</h3>
                    <p class="text-sm text-gray-400 text-left">Representamos las 5 zonas descubiertas de K-Means. <b>El tamaño de las orbes no indica la cantidad de sismos (superposición), sino el nivel de ENERGÍA CINETICA LETAL EXCESIVA (Joules) logarítmicamente focalizada en dicha área.</b> Los sismos trasparentes exponen un peligro extremo masivo.</p>
                </div>
                <div id="results-tactical" class="h-[650px] w-full rounded-b-lg mapboxgl-map"></div>
            </div>
        </section>
    ...
    <script>
        ...
        const renderPlots = () => {{
            ...
            Plotly.newPlot('results-tactical', [{{
                ...
                marker: {{ size: dfData.energy_log.map(e => Math.min(e*1.5, 18)), color: mapColorsList, opacity: 0.5 }}
            }}], {{
                ...layoutBase, mapbox: {{ style: 'carto-darkmatter', center: {{lat:4.5, lon:-74.0}}, zoom: 4.5 }}, title: '', margin: {{t:0,b:0,l:0,r:0}}, showlegend: false
            }});
        }};
